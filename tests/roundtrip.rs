...
fn sha256_known_answer_tests() {
    ...
    assert_eq!(sha256_hex(long.as_bytes()), "41edece42d63e8d9bf515a9ba6932e1c20cbc9f5a5d134645adb5db1b9737ea3");
}
...
fn row_roundtrips_even_with_delimiters_in_values() {
    ...
    let row = encode_row("BOOTHOST", &[("order", "1"), ("name", "bus"), ("note", "a|b=c\nd")]);
...
fn idx_pointer_encodes_hot_path() {
    let p = IdxPointer { pid: agt(b"slice-content"), off: 4096, len: 128 };
    let row = p.encode();
